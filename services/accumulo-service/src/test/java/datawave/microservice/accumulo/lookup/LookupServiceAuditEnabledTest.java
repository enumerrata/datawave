...
import datawave.microservice.accumulo.config.AccumuloProperties;
...
public class LookupServiceAuditEnabledTest {
    ...
    private static String EXPECTED_AUDIT_QUERYSTRING_FORMAT = "?%s&%s&query=lookup/%s/%s&auths=[A,%%20B,%%20C,%%20D,%%20E,%%20F,%%20G,%%20H,%%20I]&%s&%s&auditUserDN=userdn%%3Cissuerdn%%3E&logicClass=AccumuloLookup";
    ...
    private void setupMockAuditServer() {
        RestTemplate auditorRestTemplate = (RestTemplate) new DirectFieldAccessor(new DirectFieldAccessor(lookupService).getPropertyValue("auditor"))
    ...
    public static class TestConfiguration {
        @Bean
        public Instance accumuloInstance(AccumuloProperties accumuloProperties) {
            return new InMemoryInstance(accumuloProperties.getInstanceName());
        }
        
        @Bean
        public Connector connector(AccumuloProperties accumuloProperties, Instance accumuloInstance) throws AccumuloSecurityException, AccumuloException {
